...
set(_exported_dependencies "rclcpp;rclcpp_components;sensor_msgs;std_msgs;geometry_msgs;cv_bridge;image_transport;tf2;tf2_ros;tf2_geometry_msgs;visualization_msgs;OpenCV")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to hoop_detector_DEFINITIONS, hoop_detector_INCLUDE_DIRS,
# hoop_detector_LIBRARIES, and hoop_detector_LINK_FLAGS.
# Additionally collect the direct dependency names in
# hoop_detector_DEPENDENCIES as well as the recursive dependency names
# in hoop_detector_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(hoop_detector_DEPENDENCIES ${_exported_dependencies})
  set(hoop_detector_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(hoop_detector_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(hoop_detector_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(hoop_detector_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(hoop_detector_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(hoop_detector_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND hoop_detector_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
